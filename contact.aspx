<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage.Master" AutoEventWireup="true" CodeBehind="contact.aspx.cs" Inherits="CollegeWebsite.contact" %>
<asp:Content ID="Content1" ContentPlaceHolderID="HeadContent" runat="server">
    <style type="text/css">
    .style1
    {
        width: 100%;
    }
    .style2
    {
        width: 327px;
    }
    .style3
    {
        width: 244px;
    }
    .style5
    {
        color: white;
        font-weight: bold;
        font-family: "Times New Roman";
        font-size: x-large;
        background-color: #003366;
    }
    .style6
    {
        color: #FFFFFF;
        font-size: medium;
    }
</style>
</asp:Content>
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    <table class="style1">
    <tr>
        <td align="center" colspan="3" 
            style="color: #FFFFFF; background-color: #003366; font-weight: bold; font-size: x-large">
            Contact Us</td>
    </tr>
    <tr>
        <td class="style2" style="color: #003366; font-weight: bold">
            Name</td>
        <td class="style3" style="color: #003366; font-weight: bold">
            No.</td>
        <td style="color: #003366; font-weight: bold">
            E-mail Id</td>
    </tr>
    <tr>
        <td class="style2" 
            style="color: #FFFFFF; font-weight: bold; background-color: #003366">
            Administrator</td>
        <td class="style3" 
            style="color: #FFFFFF; font-weight: bold; background-color: #003366">
            8000711166,8573302231</td>
        <td style="color: #FFFFFF; font-weight: bold; background-color: #003366">
            viml@gmail.com</td>
    </tr>
    <tr>
        <td class="style2" 
            style="color: #FFFFFF; font-weight: bold; background-color: #003366">
            Dr. Mishra</td>
        <td class="style3" 
            style="color: #FFFFFF; font-weight: bold; background-color: #003366">
            &nbsp;</td>
        <td style="color: #FFFFFF; font-weight: bold; background-color: #003366">
            &nbsp;</td>
    </tr>
    <tr>
        <td class="style2" 
            style="color: #FFFFFF; font-weight: bold; background-color: #003366">
            Hod of MCA</td>
        <td class="style3" 
            style="color: #FFFFFF; font-weight: bold; background-color: #003366">
            &nbsp;</td>
        <td style="color: #FFFFFF; font-weight: bold; background-color: #003366">
            &nbsp;</td>
    </tr>
    <tr>
        <td class="style2" 
            style="color: #FFFFFF; font-weight: bold; background-color: #003366">
            Dr. RK Tiwari</td>
        <td class="style3" 
            style="color: #FFFFFF; font-weight: bold; background-color: #003366">
            9152518115</td>
        <td style="color: #FFFFFF; font-weight: bold; background-color: #003366">
            &nbsp;</td>
    </tr>
    <tr>
        <td class="style2" 
            style="color: #FFFFFF; font-weight: bold; background-color: #003366">
            Mete Atbas</td>
        <td class="style3" 
            style="color: #FFFFFF; font-weight: bold; background-color: #003366">
            9142290909</td>
        <td class="style5" 
            style="mso-ascii-font-family: &quot;Times New Roman&quot;; mso-bidi-font-family: &quot;Times New Roman&quot;; mso-fareast-theme-font: minor-fareast; mso-color-index: 1; mso-font-kerning: 12.0pt; language: en-US; mso-style-textfill-type: solid; mso-style-textfill-fill-themecolor: text1; mso-style-textfill-fill-color: white; mso-style-textfill-fill-alpha: 100.0%;">
            <a href="mailto:meteatbas@gmail.com"><span class="style6">
            meteatbas@gmail.com</span></a></td>
    </tr>
    <tr>
        <td class="style2" 
            style="color: #FFFFFF; font-weight: bold; background-color: #003366">
            Receptionist</td>
        <td class="style3" 
            style="color: #FFFFFF; font-weight: bold; background-color: #003366">
            05321211150</td>
        <td style="color: #FFFFFF; font-weight: bold; background-color: #003366">
            &nbsp;</td>
    </tr>
    <tr>
        <td class="style2" 
            style="color: #FFFFFF; font-weight: bold; background-color: #003366">
            &nbsp;</td>
        <td class="style3" 
            style="color: #FFFFFF; font-weight: bold; background-color: #003366">
            &nbsp;</td>
        <td style="color: #FFFFFF; font-weight: bold; background-color: #003366">
            &nbsp;</td>
    </tr>
    <tr>
        <td class="style2" 
            style="color: #FFFFFF; font-weight: bold; background-color: #003366">
            &nbsp;</td>
        <td class="style3" 
            style="color: #FFFFFF; font-weight: bold; background-color: #003366">
            &nbsp;</td>
        <td style="color: #FFFFFF; font-weight: bold; background-color: #003366">
            &nbsp;</td>
    </tr>




    </table>


</asp:Content>
